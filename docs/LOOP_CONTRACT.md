...
Stop after 5760 polls (8 real hours) if that has not happened. One correction and one negotiation per supplier. If a counter-offer has no reply after 1 sim-day, escalate the negotiation anyway. If an escalation is still open 1 sim-day after the last claim, submit that supplier with `auto_approved` false. A specifics resend restarts that wait. The loop does not call `start_exam`. After the operator starts the exam, `uv run python -m supplier_loop resume-dev` continues that round; if the server `round_id` differs from disk, local state is wiped and the new assignment is snapshotted first.
